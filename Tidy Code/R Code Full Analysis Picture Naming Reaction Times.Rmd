...
knitr::opts_knit$set(root.dir = '../Tidy Code') 
```
...
```{r Boxplot_PNrt, warning=F, message=F, echo=F} 
...
```{r Barplot_PNrt, warning=F, message=F, echo=F}
...
Skewness for action naming is 0.242 --> positively skewed but within limits (Bryne, 2010; George & Mallery, 2010)
...
actRTna.rm<- PNactions_RT[complete.cases(PNactions_RT$CR.composite.before),]
actRTna.rm<- actRTna.rm[complete.cases(actRTna.rm$zSoP.comp),]
actRTna.rm<- actRTna.rm[complete.cases(actRTna.rm$zStroop.SRC),]
actRTna.rm<- actRTna.rm[complete.cases(actRTna.rm$zWM.Score),]
...
4.3% of the variance in the data is explained by the fixed effects only. 33.7% of the variance is explained by the whole model.
...
```
0.44 -> positively skewed but within limits (Bryne, 2010; George & Mallery, 2010)
...
```{r, include=FALSE}
#Create dataset where all na.rm=T of predictors are removed from the dataset
objRTna.rm<- PNobjects_RT[complete.cases(PNobjects_RT$CR.composite.before),]
objRTna.rm<- objRTna.rm[complete.cases(objRTna.rm$zSoP.comp),]
objRTna.rm<- objRTna.rm[complete.cases(objRTna.rm$zStroop.SRC),]
objRTna.rm<- objRTna.rm[complete.cases(objRTna.rm$zWM.Score),]
...
10.5% of the variance in the data is explained by the fixed effects only. 38.7% of the variance is explained by the whole model.
...
```
*Relationship RT and Cognitive Processing Speed for reaction time (outcome) of picture naming Objects*

```{r PNobjRT_SoP_corplot, warning = F}
